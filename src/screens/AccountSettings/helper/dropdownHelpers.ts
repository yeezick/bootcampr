...
export const handleToggleDropdown = ({ setting, tempModes, dropdownModes, setDropdownModes }: ToggleDropdown) => {
  ...
  for (let key in dropdownModes) {
    ...
    else theDropdownIsAlreadyOpen ? closeDropdown(key, tempModes) : openDropdown(key, tempModes)
  }
